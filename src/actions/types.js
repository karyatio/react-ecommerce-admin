// LOGIN
export const LOGIN = "LOGIN";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAILURE = "LOGIN_FAILURE";
export const LOGOUT = "LOGOUT";
export const RESET_LOGIN = "RESET_LOGIN";

// CATALOG
export const FETCH_CATALOG = "FETCH_CATALOG";
export const FETCH_CATALOG_SUCCESS = "FETCH_CATALOG_SUCCESS";
export const FETCH_CATALOG_FAILURE = "FETCH_CATALOG_FAILURE";
export const RESET_CATALOG = "RESET_CATALOG";
export const DELETE_CATALOG_ITEM = "DELETE_CATALOG_ITEM";
export const DELETE_CATALOG_ITEM_SUCCESS = "DELETE_CATALOG_ITEM_SUCCESS";
export const DELETE_CATALOG_ITEM_FAILURE = "DELETE_CATALOG_ITEM_FAILURE";

// PRODUCTS
export const FETCH_PRODUCT = "FETCH_PRODUCT";
export const FETCH_PRODUCT_SUCCESS = "FETCH_PRODUCT_SUCCESS";
export const FETCH_PRODUCT_FAILURE = "FETCH_PRODUCT_FAILURE";
export const ADD_PRODUCT = "ADD_PRODUCT";
export const ADD_PRODUCT_SUCCESS = "ADD_PRODUCT_SUCCESS";
export const ADD_PRODUCT_FAILURE = "ADD_PRODUCT_FAILURE";
export const EDIT_PRODUCT = "EDIT_PRODUCT";
export const EDIT_PRODUCT_SUCCESS = "EDIT_PRODUCT_SUCCESS";
export const EDIT_PRODUCT_FAILURE = "EDIT_PRODUCT_FAILURE";
export const RESET_PRODUCT = "RESET_PRODUCT";

// TRANSACTIONS
export const FETCH_TRANSACTIONS = "FETCH_TRANSACTIONS";
export const FETCH_TRANSACTIONS_SUCCESS = "FETCH_TRANSACTIONS_SUCCESS";
export const FETCH_TRANSACTIONS_FAILURE = "FETCH_TRANSACTIONS_FAILURE";
export const RESET_TRANSACTIONS = "RESET_TRANSACTIONS";

// TRANSACTION DETAIL
export const FETCH_TRANSACTION_DETAIL = "FETCH_TRANSACTION_DETAIL";
export const FETCH_TRANSACTION_DETAIL_SUCCESS =
  "FETCH_TRANSACTION_DETAIL_SUCCESS";
export const FETCH_TRANSACTION_DETAIL_FAILURE =
  "FETCH_TRANSACTION_DETAIL_FAILURE";
export const CHANGE_TRANSACTION_DETAIL_STATUS =
  "CHANGE_TRANSACTION_DETAIL_STATUS";
export const CHANGE_TRANSACTION_DETAIL_STATUS_SUCCESS =
  "CHANGE_TRANSACTION_DETAIL_STATUS_SUCCESS";
export const CHANGE_TRANSACTION_DETAIL_STATUS_FAILURE =
  "CHANGE_TRANSACTION_DETAIL_STATUS_FAILURE";
export const SET_RESI = "SET_RESI";
export const SET_RESI_SUCCESS = "SET_RESI_SUCCESS";
export const SET_RESI_FAILURE = "SET_RESI_FAILURE";
export const RESET_TRANSACTION_DETAIL = "RESET_TRANSACTION_DETAIL";

// CUSTOMERS
export const FETCH_CUSTOMERS = "FETCH_CUSTOMERS";
export const FETCH_CUSTOMERS_SUCCESS = "FETCH_CUSTOMERS_SUCCESS";
export const FETCH_CUSTOMERS_FAILURE = "FETCH_CUSTOMERS_FAILURE";
export const RESET_CUSTOMERS = "RESET_CUSTOMERS";
